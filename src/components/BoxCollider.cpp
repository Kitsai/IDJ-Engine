...
namespace fyrebird {
    ...
    void BoxCollider::render() {
        ...
        Vec2 point = (Vec2(box.x, box.y) - center).rotate(_associated.angle_deg)
                     + center - cam->pos();
        points[0] = {(integer) point.x, (integer) point.y};
        points[4] = {(integer) point.x, (integer) point.y};

        point = (Vec2(box.x + box.w, box.y) - center).rotate(_associated.angle_deg)
                + center - cam->pos();
        points[1] = {(integer) point.x, (integer) point.y};

        point = (Vec2(box.x + box.w, box.y + box.h) - center).rotate(_associated.angle_deg)
                + center - cam->pos();
        points[2] = {(integer) point.x, (integer) point.y};

        point = (Vec2(box.x, box.y + box.h) - center).rotate(_associated.angle_deg)
                + center - cam->pos();
        points[3] = {(integer) point.x, (integer) point.y};
    ...
    bool BoxCollider::is_colliding(GameObject &other) {
        ...
        if (col != nullptr) {
            return Collision::IsColliding(std::static_pointer_cast<CircleCollider>(col)->circle, box,
                                          _associated.angle_deg);
        }

        col = other.get_component("BoxCollider").lock();
        return Collision::IsColliding(std::static_pointer_cast<BoxCollider>(col)->box, box, other.angle_deg,
                                      _associated.angle_deg);
    }
}